...
핵심 구성요소:
1. P4A_Config: Area-RX 매핑 및 설정 중앙 관리
2. CCM_BlockEngine: 블록별 선택적 처리 엔진
3. CCM_BlockManager: 블록 관리, 저장, 재조합
4. RXCoordinateExtractor: P1A area 설정 기반 RX 좌표 추출
...
기술 표준:
- 파일 패턴: P3A_Area{id}_RX{id}_{freq}GHz_layer_ch.npy
...
class P4A_Config:
    ...
    def __init__(self):
        """P4A_Config 초기화 (데이터 효율성 강화)"""
        # 스크립트 디렉토리 기준 경로 설정
        script_dir = os.path.dirname(os.path.abspath(__file__))
        self.p3a_data_dir = os.path.join(script_dir, "P3A_Layer_Ch_Results")
        self.ccm_save_dir = os.path.join(script_dir, "P4A_ChMeanCov_Results")
        
        # Area 필터링 설정 (None이면 모든 area 처리, 리스트로 특정 area 지정 가능)
        self.target_areas = [1]              # 예: [1, 3, 5] 또는 None
        
        # 영구 중간 계산 디렉토리 (커널 종료 시 결과 보존)
        self.ccm_working_dir = os.path.join(script_dir, "P4A_ChMeanCov_WorkingDir")
        
        # 파일 패턴 설정 (개별 파일 + 청크 파일 지원)
        self.P3A_INDIVIDUAL_PATTERN = r"Area(\d+)_(\d+(?:\.\d+)?)GHz_RX(\d+)_layer_ch\.npy"
        self.P3A_CHUNK_PATTERN = r"Area(\d+)_(\d+(?:\.\d+)?)GHz_Layer_Ch_RX(\d+)-(\d+)\.npz"
        self.CCM_FILE_PATTERN = "CCM_Area{area}_{freq}GHz.npy"
        
        # 파일 로딩 우선순위 설정
        self.PREFER_CHUNK_FILES = True   # P3A_Layer_Ch_Results에 청크 파일이 있으므로 청크 파일 우선 사용
        ...
        self.distance_filtering = {
            ...
            'max_distance_meters': 20.0,        # R_ij 계산 최대 거리 (미터)
        ...
        # 중간 결과 재사용 설정
        self.RESUME_FROM_EXISTING = True      # 기존 중간 결과 재사용 (P4A_ChMeanCov_WorkingDir/ 보존)
    ...
    def detect_area_rx_mapping(self) -> Dict[int, List[int]]:
        """P3A 파일 패턴 자동 감지: 개별 파일 + 청크 파일 지원"""
        if not os.path.exists(self.p3a_data_dir):
            raise FileNotFoundError(f"P3A 데이터 디렉토리가 존재하지 않습니다: {self.p3a_data_dir}")
        
        # 패턴 컴파일
        individual_pattern = re.compile(self.P3A_INDIVIDUAL_PATTERN)
        chunk_pattern = re.compile(self.P3A_CHUNK_PATTERN)
        
        # 임시 저장소 (우선순위 처리를 위해)
        individual_files = {}  # {(area_id, rx_id): file_info}
        chunk_files = {}       # {(area_id, rx_id): file_info}
        
        for filename in os.listdir(self.p3a_data_dir):
            file_path = os.path.join(self.p3a_data_dir, filename)
            ...
            if chunk_match:
                ...
                # 청크 파일에서 실제 RX 목록 확인
                try:
                    with np.load(file_path, allow_pickle=True) as chunk_data:
                    ...
                    for rx_id in rx_indices:
                        chunk_files[(area_id, rx_id)] = {
                            ...
                            'chunk_key': f'layer_ch_rx_{rx_id}'
                        }
    ...
    def get_area_info(self, area_id: int) -> Dict[str, Any]:
        ...
        if area_id not in self.antenna_counts:
            ...
            if file_type == 'individual':
                # 개별 파일에서 로딩 (기존 방식)
                h_data = np.load(file_path, allow_pickle=True)
                self.antenna_counts[area_id] = h_data.shape[-1]  # N_t = h_data.shape[-1]
            elif file_type == 'chunk':
                # 청크 파일에서 특정 RX 데이터 로딩
                chunk_key = file_info['chunk_key']  # 'layer_ch_rx_{rx_id}'
                with np.load(file_path, allow_pickle=True) as chunk_data:
                    if chunk_key in chunk_data:
                        chunk_item = chunk_data[chunk_key].item()  # 딕셔너리 추출
                        if 'layer_channel' in chunk_item:
                            h_data = chunk_item['layer_channel']  # 실제 채널 데이터 추출
                            self.antenna_counts[area_id] = h_data.shape[-1]  # N_t = h_data.shape[-1]
                        else:
                            raise KeyError(f"청크 데이터에서 'layer_channel' 키를 찾을 수 없습니다: {list(chunk_item.keys())}")
                    else:
...
class RXCoordinateExtractor:
    ...
    # P1A Area 설정 (P1A RT-to-Rays 코드에서 추출된 좌표 정보)
    P1_AREA_CONFIGS = {
        'area_1': {
...
class DataEfficiencyFilter:
    ...
    def __init__(self, config: P4A_Config):
        self.config = config
...
class CCM_BlockEngine:
    ...
    def process_area_ccm(self, area_id: int, progress_callback=None, start_time=None) -> Dict[str, Any]:
        ...
        # R_ii 완료 후 중간 저장 (working_dir 기반)
        ...
        # 사전 계산된 유효한 쌍들만 처리 (기존 결과 건너뛰기 지원)
        for pair_idx, (rx_i, rx_j) in enumerate(valid_r_ij_pairs):
            ...
            # R_ii 계산 시 저장했던 통계 정보 로드 (working_dir 기반)
            with np.load(os.path.join(working_dir, "stats_blocks", f'stats_{rx_i}.npz'), allow_pickle=True) as data_i:
                sigma_i = tf.constant(data_i['sigma'])
            with np.load(os.path.join(working_dir, "stats_blocks", f'stats_{rx_j}.npz'), allow_pickle=True) as data_j:
        ...
        if self.config.ENABLE_DETAILED_PROGRESS:
            ...
            # Division by zero 방지
            total_blocks = num_rx + calculated_ij_ops
            if total_blocks > 0:
                print(f"    평균 블록 처리 속도: {total_processing_time/total_blocks:.2f}초/블록")
            else:
                print(f"    평균 블록 처리 속도: N/A (처리된 블록 없음)")
        ...
        return {
            ...
            'filtering_metadata': self.efficiency_filter.get_filtering_metadata()
        }
    
    def _load_single_user(self, area_id: int, rx_id: int) -> tf.Tensor:
        """단일 사용자 레이어 채널 데이터 로딩 (P3A_Layer_Ch_Results에서, 샘플링 적용)"""
        ...
        # P3A 출력 형태: [tot_samples, L_r=1, L_t=64] (3차원)
        ...
        if self.config.sampling_config['random_sampling']:
            ...
            h_data = h_data[indices, :, :]  # 3차원 인덱싱 [tot_samples, L_r, L_t]
            ...
            # 순차 샘플링 (기존 방식)
            h_data = h_data[:symbols_sampled, :, :]  # 3차원 인덱싱 [tot_samples, L_r, L_t]
        
        # 형태 변환: [tot_samples, L_r=1, L_t] → [T, L_t] (L_r=1 제거)
        h_squeezed = h_data.squeeze(axis=1)  # [tot_samples, L_t] (L_r=1 제거)
        
        # P3A는 이미 시간-주파수가 flattened된 형태이므로 그대로 사용
        T = h_squeezed.shape[0]  # tot_samples
        L_t = h_squeezed.shape[1]  # 레이어 차원 (TX 레이어 수)
        h_reshaped = h_squeezed.reshape(T, L_t)  # [T, L_t]
    ...
    def _apply_ccm_correction(self, R_in: tf.Tensor) -> dict:
    ...
    def process_multiple_areas(self, area_ids: List[int], progress_callback=None) -> Dict[str, Any]:
        """CCM 다중 Area 통합 처리 (working_dir 기반, 영구 저장)"""
        area_results = {}
...
class CCM_BlockManager:
    ...
    def save_r_ii_intermediate(self, area_id: int, rx_ids: List[int], antenna_count: int, working_dir: str) -> str:
        ...
        # R_ii 블록들 로딩 및 저장 (working_dir 기반)
        for rx_id in rx_ids:
            r_ii_file = os.path.join(working_dir, "R_ii_blocks", f'R_ii_{rx_id}.npy')
            if os.path.exists(r_ii_file):
                data_to_save[f'R_ii_{rx_id}'] = np.load(r_ii_file, allow_pickle=True)
    ...
    def save_area_results(self, area_info: Dict[str, Any], working_dir: str):
        """임시 디렉터리의 블록 파일들을 하나의 .npz 파일로 패키징합니다. (필터링 메타데이터 포함)"""
        ...
        # R_ii 블록들 로딩 (working_dir 기반)
        for rx_id in rx_ids:
            r_ii_file = os.path.join(working_dir, "R_ii_blocks", f'R_ii_{rx_id}.npy')
            if os.path.exists(r_ii_file):
                data_to_save[f'R_ii_{rx_id}'] = np.load(r_ii_file, allow_pickle=True)
        
        # R_ij 블록들 로딩 (상삼각행렬 방식, working_dir 기반)
        for i, rx_i in enumerate(rx_ids):
            for j in range(i + 1, len(rx_ids)):
                rx_j = rx_ids[j]
                r_ij_file = os.path.join(working_dir, "R_ij_blocks", f'R_ij_{rx_i}_{rx_j}.npy')
                if os.path.exists(r_ij_file):
                    data_to_save[f'R_ij_{rx_i}_{rx_j}'] = np.load(r_ij_file, allow_pickle=True)
        
        # 필터링 메타데이터 추가
        if 'filtering_metadata' in area_info:
            data_to_save['filtering_stats'] = str(area_info['filtering_metadata'])
    ...
    def load_area_results(self, area_id: int):
        """Area 결과 파일 로드 (블록 저장 방식 지원)"""
        # P4A_Config 경로에서 블록별 결과 복원 (우선순위: ALL_RXs -> 일반)
...
def run_correction_mode(ccm_dir: str, config: P4A_Config):
    """
    ccm_dir의 R_ii 행렬을 검사하고, 유효하지 않으면 원본 데이터로
    재계산 및 보정을 거쳐 원본 ccm 파일을 덮어씁니다. (필터링 지원)
    """
    correction_start_time = time.time()
    print(f"--- P4A CCM 블록 보정 모드 시작 ---")
    print(f"대상 디렉터리: {ccm_dir}")

    # target_areas 설정 정보 표시
    if config.target_areas is None:
        print(f"보정 범위: 모든 Area")
    else:
        areas_str = ', '.join([f"Area{area}" for area in sorted(config.target_areas)])
        print(f"보정 범위: {areas_str} (target_areas 설정 적용)")
    
    available_areas = list(config.area_rx_mapping.keys())
    if available_areas:
        print(f"처리 가능한 Area: {sorted(available_areas)}")
    ...
    processed_files = 0  # 실제 처리된 파일 수
    file_width = len(str(total_files))
    
    print(f"발견된 CCM 파일: {total_files}개")
    ...
    for i, ccm_filename in enumerate(ccm_files):
        ...
        # target_areas 설정에 따른 필터링 - 매핑에 없는 Area는 건너뛰기
        if area_id not in config.area_rx_mapping:
            print(f"Area {area_id}는 target_areas 설정에 포함되지 않음 - 보정 건너뜀")
            continue

        processed_files += 1
        ccm_filepath = os.path.join(ccm_dir, ccm_filename)
        file_start_time = time.time()
        print(f"\n[처리 {processed_files}] Area {area_id} 파일 검사: {ccm_filename}")
        
        try:
            data = np.load(ccm_filepath, allow_pickle=True)
    ...
    print(f"\n--- P4A CCM 블록 보정 모드 완료 ---")
    print(f"검사한 파일: {processed_files}개")
    print(f"수정된 파일: {corrected_file_count}개")
    print(f"수정된 블록: {total_corrected_blocks}개")
    if total_files != processed_files:
        print(f"건너뛴 파일: {total_files - processed_files}개 (target_areas 설정에 의해)")
    print(f"전체 소요 시간: {format_time(time.time() - correction_start_time)}")


def main_p4a_v1_pipeline(config: P4A_Config):
    """P4A 메인 파이프라인 (v1 호환 스타일)"""
    ...
    if config.execution_mode == 'correct':
        ...
        print("\n=== P4A CCM 블록 [저장 모드 - 메모리 최적화] ===")
        results = process_ccm_pipeline_v1(area_list, config)
        return results
...
def process_ccm_pipeline_v1(area_ids: List[int] = None, config: P4A_Config = None) -> Dict[str, Any]:
    """P4A v1.0 메인 파이프라인: 데이터 효율성 최적화"""
    ...
    for area_id in area_ids:
        ...
        # 영구 working directory 사용 (임시 디렉토리 제거)
        # CCM 블록 처리
...
if __name__ == "__main__":
    ...
    try:
        ...
        if available_areas:
            # 메인 파이프라인 실행
            result = main_p4a_v1_pipeline(config)
        else:
            print("처리 가능한 P3A 레이어 채널 데이터가 없습니다.")
            print("P3A_Layer_Ch_Results 디렉토리에 P3A 레이어 채널 파일을 확인하세요.")
