...
"""
P2B_BF_Analysis.py
P2B: 빔포밍 결과 분석 및 TX/RX 빔 클러스터링

P2A_BF_Pair에서 생성된 빔포밍 채널 이득 데이터를 분석하여
...
# ========================================================================
# DATA LOADING - P2A 결과 로딩
# ========================================================================

class P2ADataLoader:
    """P2A 결과 파일 로딩"""
    
    def __init__(self, config: P2B_Config):
        self.config = config
    
    def load_p2a_results(self) -> Dict[str, Any]:
        """P2A 결과 파일들을 로딩하여 통합 데이터 구조로 반환"""
        area = self.config.TARGET_AREA
        freq = self.config.TARGET_FREQUENCY
        
        print(f"Loading P2A results for Area{area}_{freq}GHz...")
        ...
        print(f"P2A data loading completed: {success_count}/{self.config.TOTAL_RX_COUNT} RX loaded")
        
        if success_count == 0:
            raise FileNotFoundError(f"No P2A data found for Area{area}_{freq}GHz")
...
class BaseBeamClusteringAnalyzer:
    ...
    def __init__(self, config: P2B_Config, beam_type: str):
        self.config = config
...
class ResultsExporter:
    ...
    def generate_summary_report(self, analysis_results: Dict[str, Any], 
        ...
        report_lines.append(f"P2B 빔포밍 성능 분석 리포트")
...
class P2BPipeline:
    """P2B 메인 실행 파이프라인 (TX/RX 빔 클러스터링)"""
    
    def __init__(self):
        self.config = config
        self.data_loader = P2ADataLoader(self.config)
    ...
    def execute_analysis(self):
        """전체 P2B 분석 파이프라인 실행 (TX/RX 빔 클러스터링 포함)"""
        
        print("=" * 80)
        print("P2B: TX/RX 빔 클러스터링 분석 파이프라인 시작")
        print(f"Target: Area{self.config.TARGET_AREA}_{self.config.TARGET_FREQUENCY}GHz")
        print("=" * 80)
        
        try:
            # 1. P2A 결과 데이터 로딩
            print("\n[Step 1/6] P2A 결과 데이터 로딩")
            p2a_results = self.data_loader.load_p2a_results()
            
            # 2. P1A 수신기 위치 정보 생성
            print("\n[Step 2/6] P1A 수신기 위치 정보 생성")
            rx_positions = self.position_generator.generate_area1_rx_positions()
            
            # 3. 빔 순위 분석
            print("\n[Step 3/6] 빔 순위 분석 실행")
            analysis_results = self.beam_analyzer.rank_beam_pairs(p2a_results, rx_positions)
            ...
            print(f"Error during P2B analysis: {e}")
            raise
...
def main():
    """메인 실행 함수"""
    try:
        pipeline = P2BPipeline()
        result = pipeline.execute_analysis()
        return result
    except Exception as e:
        print(f"P2B TX/RX 빔 클러스터링 분석 실패: {e}")
