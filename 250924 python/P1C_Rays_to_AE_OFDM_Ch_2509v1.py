# ======================================================================
# P1C_Rays_to_AE_OFDM_Ch_2509.py
# P1C: Ray Data → Antenna Element OFDM Channel 변환
# 원본: P2_Rays_to_OFDM_Ch 기반으로 P1C에 맞게 수정
# 
# P1C 특징:
# - P1B Valid RXs 결과를 입력으로 사용
# - Antenna Element별 OFDM 채널 생성
...
# ===== SECTION 2: P1C_Config (P1B Valid RXs 기반 설정) =====
class P1C_Config:
    ...
    def __init__(self):
        ...
        self.P1C_OUTPUT_DIR = os.path.join(script_dir, "P1C_AE_OFDM_Ch_Results")
        ...
        self.CHUNK_PATTERN = "Area{area}_{freq}GHz_AE_OFDM_Ch_RX{start}-{end}.npz"
        ...
        self.OFDM_FFT = 1                              # 원본: N_FFT → 결과 OFDM 채널의 부반송파 수
        ...
        # 채널 생성 파라미터
        self.static_ch_realizations = 1000              # 독립 정적 채널 개수
    ...
    def create_chunk_from_files(self, area_index, frequency, rx_indices):
        ...
        for idx, rx_idx in enumerate(rx_indices, 1):
            individual_file = os.path.join(self.P1C_OUTPUT_DIR, 
                                         self.P1C_FILE_PATTERN.format(area=area_index, freq=frequency, rx=rx_idx))
        ...
        chunk_filepath = os.path.join(self.P1C_OUTPUT_DIR, chunk_filename)
    ...
    def print_p1c_config(self):
        """P1C 설정 정보 출력"""
        print(f"P1C Antenna Element OFDM 채널 생성 설정:")
        ...
        print(f"  - 예상 총 심볼 수 (per file): {self.static_ch_realizations * self.doppler_time_realizations * self.doppler_sym_ofdm* self.OFDM_FFT}")
...
class ChCoeGen(Object):
    """P1C: Ray → Antenna Element OFDM Channel 변환 (P1B Valid RXs 기반)
    ...
    - P1C: Valid RXs → Antenna Element OFDM 채널
    """
...
# ===== SECTION 4: 메인 실행 P1C (P1B Valid RXs 기반 Antenna Element OFDM 채널 생성) =====
def main():
    """P1C: P1B Valid RXs를 기반으로 Antenna Element OFDM 채널 생성"""
    ...
    for area_index, fc, RX_index in config.p1b_data_combinations:
        ...
        os.makedirs(config.P1C_OUTPUT_DIR, exist_ok=True)
        output_filename = config.P1C_FILE_PATTERN.format(area=area_index, freq=fc, rx=RX_index)
        np.save(f'{config.P1C_OUTPUT_DIR}/{output_filename}', 
        ...
        if config.ENABLE_SCREEN_CLEAR and (rx_file_count % config.PROGRESS_CLEAR_INTERVAL == 0 or rx_file_count == total_rx_files):
            ...
            # 진행 상황 요약 재출력
            print("P1C Antenna Element OFDM 채널 생성 진행률:")
            print(f"✓ [{rx_file_count:>{total_rx_width}}/{total_rx_files}] {output_filename}")
    ...
    if config.ENABLE_POST_CHUNK:
        ...
        print(f"총 {total_chunks_created}개 청크 생성 완료 (총 {total_rx_processed}개 Valid RX 처리)")
