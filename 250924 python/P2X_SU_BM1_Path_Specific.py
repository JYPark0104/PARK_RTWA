...
# P2X: P1B 필터링된 Ray Data → Path-Specific Analysis
# 
# 기능:
# - 레이어 기반 빔포밍 채널 이득 계산
# - 2차원 크로네커 DFT 코드북 생성
...
# ===== SECTION 2: P2X_Config (레이어 기반 빔포밍 설정) =====
class P2X_Config:
    ...
    def __init__(self):
        ...
        self.P2X_OUTPUT_DIR = os.path.join(script_dir, "P2X_BM1_Results")
        self.P2X_FILE_PATTERN = "Area{area}_{freq}GHz_RX{rx}_bm1_path_specific.npz"
    ...
    def create_chunk_from_files(self, area_index, frequency, rx_indices):
        ...
        for idx, rx_idx in enumerate(rx_indices, 1):
            individual_file = os.path.join(self.P2X_OUTPUT_DIR, 
                                         self.P2X_FILE_PATTERN.format(area=area_index, freq=frequency, rx=rx_idx))
        ...
        chunk_filepath = os.path.join(self.P2X_OUTPUT_DIR, chunk_filename)
...
def generate_h_ae_batch(topology, ray_pdap, config, OFDM_ChGen):
    ...
    """
    # Static 채널 생성
    h_delay_bin_static, aoa_delay_bin, zoa_delay_bin, aod_delay_bin, zod_delay_bin = OFDM_ChGen._compute_ch_mimo_ofdm_38901_static(
    ...
    aod_delay_bin = tf.transpose(aod_delay_bin, [0,3,1,2,4])
    zod_delay_bin = tf.transpose(zod_delay_bin, [0,3,1,2,4])


    # aod_delay_bin, zod_delay_bin은 추후 사용을 위해 추가됨 (현재 미사용) 
    ...
    for doppler_idx in range(config.doppler_time_realizations):
        ...
        for sym_idx in range(config.doppler_sym_ofdm):
            ...
            # 도플러 효과 적용 (기존 로직 유지: aoa_delay_bin, zoa_delay_bin만 사용)
...
class ChCoeGen(Object):
    """P2X: Ray → 빔포밍 채널 이득 계산 (독립 정적채널 + 도플러 시변채널)
    ...
    - P2X: 빔포밍 채널 이득 계산 → 빔 페어별 이득 + 방향 정보
    """
    ...
    def _compute_ch_mimo_ofdm_38901_static(self, topology, rays, fft_size, scs):
        ...
        Returns
        -------
        h_delay_bin_static : tf.Tensor
            정적 채널 계수 (delay bin domain)
        aoa_delay_bin : tf.Tensor  
            AoA delay bin 확장 데이터
        zoa_delay_bin : tf.Tensor
            ZoA delay bin 확장 데이터
        aod_delay_bin : tf.Tensor
            AoD delay bin 확장 데이터
        zod_delay_bin : tf.Tensor
            ZoD delay bin 확장 데이터
        """
        ...
        return h_delay_bin_static, aoa_delay_bin, zoa_delay_bin, aod_delay_bin, zod_delay_bin
...
def main():
    """P2X: 빔포밍 채널 이득 계산을 통한 TX-RX 빔 페어 성능 분석"""
    ...
    os.makedirs(config.P2X_OUTPUT_DIR, exist_ok=True)
    ...
    for area_index, fc in sorted(area_freq_combinations):
        ...
        codebook_filepath = f'{config.P2X_OUTPUT_DIR}/{codebook_filename}'
    ...
    for area_index, fc, RX_index in config.p1b_data_combinations:
        ...
        output_filename = config.P2X_FILE_PATTERN.format(area=area_index, freq=fc, rx=RX_index)
        ...
        # 간소화된 npz 저장
        np.savez(f'{config.P2X_OUTPUT_DIR}/{output_filename}',
