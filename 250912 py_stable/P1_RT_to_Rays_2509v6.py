#!/usr/bin/env python3
"""
RT to Paths & Rays Integrated Pipeline (250806v4.2)
...
v4.2 New Features (P2 Compatibility Enhancement):
...
- Part 3: P1_RT_to_Paths.ipynb (TX/RX positioning reference)
"""
...
class P1_Config:
    ...
    # File Management (v4 Enhanced - Unified folder structure)
    SCENE_FILE_PATHS = ['../Jonggak/Jonggak.xml', 'Jonggak/Jonggak.xml', './Jonggak/Jonggak.xml']
    UNIFIED_SAVE_DIR = "Sionna_RT_Results"                  # Single main directory for all data
    # File naming: Area{X}_{freq}GHz_{Path|Ray|ChannelBlock}_RX{Y}_{type}.npy within unified directory
    
    # Data Type Configuration (v4.2 Enhancement for P2 compatibility)
    ...
    ENABLE_PATH_SAVING = False     # Path data kept in memory only (not saved to disk)
    ...
    # Preview Mode Configuration (v4 Enhanced)
    PREVIEW_ONLY_MODE = True     # True: Only show TX/RX placement visualization (no RT computation)
    
    
    # ========== SECTION 2: EXPERIMENTAL PARAMETERS ==========
    # Data I/O and processing configuration (most frequently changed)
    
    # Multi-area & Multi-frequency Configuration (v4.1 Enhanced)
    ...
    # Area-based TX/RX Configuration (v4 Enhanced - from P1_RT_to_Paths.ipynb)
    ...
    # RX Selection Configuration (v4 Enhanced)
...
if p1_config.RANDOM_SEED != 1:
    tf.random.set_seed(p1_config.RANDOM_SEED)
...
class Utils:
    ...
    def rad_to_deg(angle_rad):
        """Convert radians to degrees (Part 1 Cell 7, Part 2 Cell 2)"""
        return angle_rad * 180.0 / np.pi
    
    @staticmethod
    def deg_to_rad(angle_deg):
        """Convert degrees to radians (Part 2 Cell 2)"""
        return angle_deg * np.pi / 180.0
    
    @staticmethod
    def get_area_config(area_index):
        """Safely get area configuration with validation (v4 enhancement)"""
        area_key = f'area_{area_index}'
        if area_key not in p1_config.AREA_CONFIGS:
            raise ValueError(f"Area {area_index} not found in AREA_CONFIGS. Available areas: {list(p1_config.AREA_CONFIGS.keys())}")
        return p1_config.AREA_CONFIGS[area_key]
    ...
    def compute_ray_statistics(ray_data, param_name):
        ...
        elif param_name in ['theta_r', 'theta_t', 'phi_r', 'phi_t']:
            # Angle-specific: angular spread
    ...
    def create_metadata(path_data_summary, ray_data_summary, run_config):
        ...
        metadata = {
            "session_info": {
                ...
                "pipeline_version": "P1_RT_to_Paths_Rays_250806v4",
            ...
            "configuration": {
                ...
                "enable_path_saving": p1_config.ENABLE_PATH_SAVING,
                "enable_ray_saving": p1_config.ENABLE_RAY_SAVING,
                "enable_channel_block_saving": p1_config.ENABLE_CHANNEL_BLOCK_SAVING
            },
            "path_data_summary": path_data_summary,
            "ray_data_summary": ray_data_summary,
            "processing_parameters": {
                "max_interesting_paths": p1_config.NUM_INTERESTING_PATHS,
                "max_depth": p1_config.MAX_DEPTH,
                "max_rays_per_pair": p1_config.MAX_RAYS_PER_PAIR,
                "random_seed": p1_config.RANDOM_SEED,
                "rt_seed": p1_config.RT_SEED
            },
            "area_configurations": {},
...
class Scene:
    ...
    def find_file():
        ...
        for path in p1_config.SCENE_FILE_PATHS:
            if os.path.exists(path):
                return path
        
        print(f"Current working directory: {os.getcwd()}")
        print("Available directories:", [d for d in os.listdir('.') if os.path.isdir(d)])
        raise FileNotFoundError(f"Scene file not found. Tried paths: {p1_config.SCENE_FILE_PATHS}")
    ...
    def setup_antennas(scene):
        ...
        antenna_pattern = PlanarArray(
            num_rows=p1_config.ANTENNA_NUM_ROWS, 
            num_cols=p1_config.ANTENNA_NUM_COLS, 
            vertical_spacing=p1_config.ANTENNA_VERTICAL_SPACING,
            horizontal_spacing=p1_config.ANTENNA_HORIZONTAL_SPACING, 
            pattern=p1_config.ANTENNA_PATTERN, 
            polarization=p1_config.ANTENNA_POLARIZATION
        )
    ...
    def tune_materials(scene, target=None, s_coeff=None, xpd_coeff=None):
        ...
        if target is None:
            target = p1_config.ITU_TARGET_MATERIALS
        if s_coeff is None:
            s_coeff = p1_config.ITU_SCATTERING_COEFF
        if xpd_coeff is None:
            xpd_coeff = p1_config.ITU_XPD_COEFF
    ...
    def add_transmitters(scene, area_index):
        """Deploy transmitters based on selected area configuration (Part 1 Cell 5 + v4 enhancement)"""
        ...
        for tx_position in tx_positions:
            ...
            scene.add(Transmitter(f"tx{tx_index}", tx_position, 
                                orientation=p1_config.TX_ORIENTATION, 
                                display_radius=p1_config.TX_DISPLAY_RADIUS))
    
    @staticmethod
    def add_receivers(scene, area_index):
        """Deploy receivers based on area grid configuration (Part 1 Cell 5 + v4 Area-based enhancement)"""
        ...
        for rx_position in rx_positions:
            ...
            scene.add(Receiver(f"rx{rx_index}", rx_position, 
                             orientation=p1_config.RX_ORIENTATION, 
                             display_radius=p1_config.RX_DISPLAY_RADIUS))
    
    @staticmethod
    def preview(scene):
        """Generate scene preview if ENABLE_SCENE_PREVIEW is True (Part 1 Cell 5)"""
        if p1_config.ENABLE_SCENE_PREVIEW:
    ...
    def generate_area_rx_grid(area_index):
        """Generate RX grid positions for selected area using systematic placement methods (v4.3 enhancement)"""
        area_config = Utils.get_area_config(area_index)
...
class PathRT:
    ...
    def setup_rf(frequency_ghz):
        """Set RF frequency for specific frequency (Part 1 Cell 2, simplified v4)"""
    ...
    def trace_rays(scene, carrier_frequency, frequency_ghz):
        ...
        p_solver = PathSolver()
        paths = p_solver(scene=scene, max_depth=p1_config.MAX_DEPTH, los=True, specular_reflection=True, 
                        diffuse_reflection=True, refraction=True, synthetic_array=False, seed=p1_config.RT_SEED)
    ...
    def compute_angles(a_, tau, paths):
        ...
        # Cell 7: Extract and reshape angle tensors to match CIR format
        theta_r = PathRT.reshape_angles(paths.theta_r)
        phi_r = PathRT.reshape_angles(paths.phi_r)
        theta_t = PathRT.reshape_angles(paths.theta_t)
        phi_t = PathRT.reshape_angles(paths.phi_t)
        
        # Cell 7: Convert to degrees
        theta_r = Utils.rad_to_deg(theta_r)
        phi_r = Utils.rad_to_deg(phi_r)
        theta_t = Utils.rad_to_deg(theta_t)
        phi_t = Utils.rad_to_deg(phi_t)
        
        # Cell 7: Original notebook works with amplitude (a_), not power
        return a_, theta_r, phi_r, theta_t, phi_t
    
    @staticmethod
    def sort_by_power(a_, tau, theta_r, phi_r, theta_t, phi_t, path_types, vertices):
        ...
        theta_r_sorted = tf.gather(theta_r, sorted_indices, axis=2, batch_dims=2)
        phi_r_sorted = tf.gather(phi_r, sorted_indices, axis=2, batch_dims=2)
        theta_t_sorted = tf.gather(theta_t, sorted_indices, axis=2, batch_dims=2)
        phi_t_sorted = tf.gather(phi_t, sorted_indices, axis=2, batch_dims=2)
        ...
        return (a_sorted, tau_sorted, theta_r_sorted, phi_r_sorted, 
                theta_t_sorted, phi_t_sorted, path_types_sorted, vertices_sorted)
    
    @staticmethod
    def select_top(sorted_data, num_paths):
        """Select strongest N paths from sorted data (Part 1 Cell 7)"""
        (a_sorted, tau_sorted, theta_r_sorted, phi_r_sorted, 
         theta_t_sorted, phi_t_sorted, path_types_sorted, vertices_sorted) = sorted_data
        ...
        selected_data = (
            ...
            theta_r_sorted[:, :, 0:num_paths],
            phi_r_sorted[:, :, 0:num_paths],
            theta_t_sorted[:, :, 0:num_paths],
            phi_t_sorted[:, :, 0:num_paths],
...
class DataIO:
    ...
    def save_paths_unified(arrays_dict, carrier_frequency, area_index, rx_index):
        """Save path data arrays with unified folder and descriptive filenames (Part 1 Cell 7 + v4 enhancement)"""
        directory = p1_config.UNIFIED_SAVE_DIR
    ...
    def save_all_rx_paths_unified(selected_data, carrier_frequency, area_index, scene):
        """Save path data for all RX or selected RX indices with unified folder structure (Part 1 Cell 7 + v4 enhancement)"""
        if p1_config.ENABLE_PATH_SAVING:
            (power_selected, tau_selected, theta_r_selected, phi_r_selected, 
             theta_t_selected, phi_t_selected, path_types_selected, vertices_selected) = selected_data
            
            # Determine which RX indices to save
            num_rx = len(scene.receivers)
            if p1_config.SAVE_ALL_RX:
                rx_indices_to_save = list(range(1, num_rx + 1))
                print(f"Area {area_index}: saving {num_rx} RX data files")
            else:
                rx_indices_to_save = p1_config.SELECTED_RX_INDICES
            ...
            for rx_idx in rx_indices_to_save:
                if rx_idx <= num_rx:
                    ...
                    arrays = {
                        ...
                        'theta_r': theta_r_selected[:, rx_array_idx:rx_array_idx+1, :],
                        'phi_r': phi_r_selected[:, rx_array_idx:rx_array_idx+1, :],
                        'theta_t': theta_t_selected[:, rx_array_idx:rx_array_idx+1, :],
                        'phi_t': phi_t_selected[:, rx_array_idx:rx_array_idx+1, :],
    ...
    @staticmethod
    def load_paths_v4(area_index, freq_ghz, rx_index):
        """Load saved path data arrays with v4 unified folder structure (Part 2 Cell 2 + v4 enhancement)"""
        try:
            base_dir = p1_config.UNIFIED_SAVE_DIR
            ...
            path_data = {
                ...
                'theta_r': np.load(f"{base_dir}/{base_filename}_theta_r.npy"),
                'theta_t': np.load(f"{base_dir}/{base_filename}_theta_t.npy"),
                'phi_r': np.load(f"{base_dir}/{base_filename}_phi_r.npy"),
                'phi_t': np.load(f"{base_dir}/{base_filename}_phi_t.npy"),
    ...
    @staticmethod
    def save_rays_v4(ray_arrays, area_index, freq_ghz, rx_index):
        """Save ray data to unified folder with v4 naming (Part 2 Cell 2 + v4 enhancement)"""
        if p1_config.ENABLE_RAY_SAVING:
            save_dir = p1_config.UNIFIED_SAVE_DIR
            os.makedirs(save_dir, exist_ok=True)
            
            # Save basic ray arrays with v4 naming (v4.2: float32 optimization for P2 compatibility)
            for key in ['tau', 'power', 'theta_r', 'theta_t', 'phi_r', 'phi_t', 'counts']:
                filename = f"Area{area_index}_{freq_ghz}GHz_Ray_RX{rx_index}_{key}.npy"
                filepath = os.path.join(save_dir, filename)
                
                # v4.2 Enhancement: Force float32 for P2 compatibility and memory efficiency
                if p1_config.FORCE_FLOAT32_SAVING and key != 'counts':
                    # Ray parameters: convert to float32 for 50% memory saving and P2 compatibility
                    np.save(filepath, ray_arrays[key].astype(p1_config.RAY_DATA_DTYPE))
                    print(f"Ray {key} saved (float32): {filepath}")
                else:
                    # Counts: keep original int32 dtype
                    np.save(filepath, ray_arrays[key])
                    print(f"Ray {key} saved: {filepath}")
            
            # Convert to TensorFlow and save channel block format with v4 naming (optional)
            if p1_config.ENABLE_CHANNEL_BLOCK_SAVING:
                DataIO.save_channel_blocks_v4(ray_arrays, area_index, freq_ghz, rx_index)
            else:
    ...
    def compute_and_store_ray_statistics(ray_arrays, metadata, area_index, freq_ghz, rx_index):
        ...
        ray_params = ['tau', 'power', 'theta_r', 'theta_t', 'phi_r', 'phi_t']
        ...
        if 'counts' in ray_arrays:
            metadata["ray_statistics_summary"][key_prefix]["ray_counts"] = {
                ...
                "max_rays_per_pair": int(p1_config.MAX_RAYS_PER_PAIR),
                "utilization_ratio": float(ray_arrays['counts'].sum() / p1_config.MAX_RAYS_PER_PAIR)
            }
    ...
    def save_channel_blocks(ray_arrays, save_dir, freq_ghz, rx_index):
        ...
        phi_r_rays_tf = tf.Variable(ray_arrays['phi_r'], dtype=tf.float32)
        phi_t_rays_tf = tf.Variable(ray_arrays['phi_t'], dtype=tf.float32)
        theta_r_rays_tf = tf.Variable(ray_arrays['theta_r'], dtype=tf.float32)
        theta_t_rays_tf = tf.Variable(ray_arrays['theta_t'], dtype=tf.float32)
    ...
    @staticmethod
    def save_channel_blocks_v4(ray_arrays, area_index, freq_ghz, rx_index):
        """Convert and save ray data in channel block format with v4 naming (Part 2 Cell 2 + v4 enhancement)"""
        # Convert to TensorFlow variables
        tau_rays_tf = tf.Variable(ray_arrays['tau'], dtype=tf.float32)
        power_rays_tf = tf.Variable(ray_arrays['power'], dtype=tf.float32)
        phi_r_rays_tf = tf.Variable(ray_arrays['phi_r'], dtype=tf.float32)
        ...
        # Save channel block format with v4 naming
        save_dir = p1_config.UNIFIED_SAVE_DIR
        channel_block_files = {
            'tau_rays_for_ChannelBlock': tau_for_channel_block,
            'power_rays_for_ChannelBlock': power_for_channel_block,
    ...
    def save_metadata_json(metadata, timestamp):
        ...
        if p1_config.ENABLE_METADATA_SAVING:
            save_dir = p1_config.UNIFIED_SAVE_DIR
            os.makedirs(save_dir, exist_ok=True)
            
            filename = f"P1_Ray_Metadata_{timestamp}.json"
            filepath = os.path.join(save_dir, filename)
    ...
    def save_all_rays_unified_npz(all_ray_data, area_index, freq_ghz):
        ...
        freq_ghz : float
            Frequency in GHz
        """
        if p1_config.ENABLE_RAY_SAVING and all_ray_data:
            save_dir = p1_config.UNIFIED_SAVE_DIR
            os.makedirs(save_dir, exist_ok=True)
            ...
            # Ray parameters to save
            ray_params = ['tau', 'power', 'theta_r', 'theta_t', 'phi_r', 'phi_t', 'counts']
            ...
            for param in ray_params:
                ...
                for rx_idx in rx_indices:
                    if param in all_ray_data[rx_idx]:
                        ...
                        # Convert to float32 for P2 compatibility (except counts)
                        if p1_config.FORCE_FLOAT32_SAVING and param != 'counts':
                            ray_data = ray_data.astype(p1_config.RAY_DATA_DTYPE)
        ...
        for param in channel_block_params:
            ...
            for rx_idx in rx_indices:
                ...
                if param.startswith('tau'):
                    ...
                    ray_data = Utils.deg_to_rad(ray_arrays['phi_r'])
                elif param.startswith('phi_t'):
                    ray_data = Utils.deg_to_rad(ray_arrays['phi_t'])
                elif param.startswith('theta_r'):
                    ray_data = Utils.deg_to_rad(ray_arrays['theta_r'])
                elif param.startswith('theta_t'):
                    ray_data = Utils.deg_to_rad(ray_arrays['theta_t'])
...
class RayGen:
    ...
    def __init__(self, carrier_frequency, los_scenario):
        """Initialize ray generator with TR 38.901 parameters (Part 2 Cell 2, simplified v4)"""
        ...
        self.c_ZSA = p1_config.TR38901_C_ZSA
        self.mu_lgZSD = p1_config.TR38901_MU_LGZSD
        self.c_ZSD = 3 / 8 * 10 ** (self.mu_lgZSD)
        
        # Set scenario-specific parameters from TR 38.901
        if los_scenario == 1:  # LOS
            self.c_ASA = p1_config.LOS_C_ASA
            self.c_ASD = p1_config.LOS_C_ASD
            self.r_tau = p1_config.LOS_R_TAU
            self.N = p1_config.LOS_N_CLUSTERS
            self.DS = -6.955 - 0.0963*math.log10(carrier_frequency)
        else:  # NLOS
            self.c_ASA = p1_config.NLOS_C_ASA
            self.c_ASD = p1_config.NLOS_C_ASD
            self.r_tau = p1_config.NLOS_R_TAU
            self.N = p1_config.NLOS_N_CLUSTERS
    ...
    def subrayProposed(self, path_interactions, los_scenario, tau_cluster, 
        ...
        if los_scenario == 1:
            comb_n = p1_config.LOS_COMB_N
        else:
            comb_n = p1_config.NLOS_COMB_N
    ...
    def initialize_storage():
        ...
        Enhanced P1 v4.1: TR 38.901 compatible storage format
        ...
        num_tx = p1_config.N_BS      # N_BS (Base Stations)
        num_rx = p1_config.N_UE      # N_UE (User Equipment)
        cluster_dim = p1_config.CLUSTER_DIM # Cluster dimension (TR 38.901 requirement)
        max_rays = p1_config.MAX_RAYS_PER_PAIR
        
        if p1_config.USE_SIONNA_FORMAT:
        ...
        ray_arrays = {
            ...
            'phi_r': np.zeros(ray_shape),
            'phi_t': np.zeros(ray_shape),
            'theta_r': np.zeros(ray_shape),
            'theta_t': np.zeros(ray_shape),
            'counts': np.zeros(counts_shape, dtype=np.int32)
        }
        
        print(f"[P1 v4.1] Initialized {format_name} storage: {ray_shape}")
        print(f"[P1 v4.1] Batch processing: {p1_config.BATCH_PROCESSING}, Batch size: {batch_size}")
        print(f"[P1 v4.1] Sionna format: {p1_config.USE_SIONNA_FORMAT}")
    ...
    def generate_for_pairs(path_data, carrier_frequency, ray_arrays, num_tx, num_rx):
        ...
        theta_r_sorted = path_data['theta_r']
        theta_t_sorted = path_data['theta_t']
        phi_r_sorted = path_data['phi_r']
        phi_t_sorted = path_data['phi_t']
        ...
        for tx in range(num_tx):
            for rx in range(num_rx):
                ...
                rays_from_paths = RayGen.generate_from_paths(
                    ...
                    tau_sorted, phi_r_sorted, phi_t_sorted, 
                    theta_r_sorted, theta_t_sorted, power_sorted, tx, rx
                )
    ...
    def generate_from_paths(ray_generator, path_types, los_scenario, tau_sorted, 
                           phi_r_sorted, phi_t_sorted, theta_r_sorted, theta_t_sorted, 
                           power_sorted, tx, rx):
        """Generate rays from all paths for specific TX-RX pair (Part 2 Cell 2)"""
        all_tau_rays = []
        all_power_rays = []
        all_phi_r_rays = []
        all_phi_t_rays = []
        ...
        for path_idx in range(num_paths):
            # Cell 2: Extract path data exactly as in original notebook (line 133-145)
            path_interactions = path_types[tx, rx, path_idx, :]
            path_tau = tau_sorted[tx, rx, path_idx]
            path_phi_r = phi_r_sorted[tx, rx, path_idx]
            path_phi_t = phi_t_sorted[tx, rx, path_idx]
            path_theta_r = theta_r_sorted[tx, rx, path_idx]
            path_theta_t = theta_t_sorted[tx, rx, path_idx]
            path_power = power_sorted[tx, rx, path_idx]
            
            # Cell 2: Call RayGen.subrayProposed exactly as in original (line 136-145)
            ...
            # Cell 2: Store rays following original notebook logic (line 148-161)
        ...
        return {
            ...
            'phi_r': np.array(all_phi_r_rays),
            'phi_t': np.array(all_phi_t_rays),
            'theta_r': np.array(all_theta_r_rays),
            'theta_t': np.array(all_theta_t_rays)
        }
    
    @staticmethod
    def store_rays(rays_from_paths, ray_arrays, tx, rx, max_rays_per_pair):
        """Store generated rays in ray storage arrays with power-based selection (Part 2 Cell 2)"""
        ...
        if total_rays > max_rays_per_pair:
            ...
            for key in ['tau', 'power', 'phi_r', 'phi_t', 'theta_r', 'theta_t']:
                if p1_config.USE_SIONNA_FORMAT:
...
class Pipeline:
    ...
    def execute_preview_only():
        """AREA_CONFIGS 기반 TX/RX 배치 3D 시각화 프리뷰 (v4 enhancement)"""
        print("=" * 80)
        print("AREA_CONFIGS Visual Preview Mode (v4)")
        ...
        try:
            ...
            areas = p1_config.AREA_INDICES
            frequencies = p1_config.FREQUENCY_CONFIGS
    ...
    @staticmethod
    def execute_main():
        """Main execution pipeline with memory-based path-to-ray processing (v4.1 Enhanced)"""
        
        # Check for preview-only mode
        if p1_config.PREVIEW_ONLY_MODE:
            Pipeline.execute_preview_only()
            return
        
        print("=" * 80)
        print("P1 RT to Paths & Rays Integrated Pipeline (250806v4.1)")
        print("Multi-area & Multi-frequency with TR 38.901 Batch Processing")
        print("=" * 80)
        
        # TR 38.901 Batch Processing Configuration
        batch_size = p1_config.BATCH_SIZE
        batch_processing = p1_config.BATCH_PROCESSING
        
        if batch_processing and batch_size > 1:
            print(f"[P1 v4.1] Batch processing enabled: {batch_size} batches")
            for batch_idx in range(batch_size):
                print(f"\n=== BATCH {batch_idx + 1}/{batch_size} ===")
                Pipeline.execute_single_batch(batch_idx)
        else:
            print(f"[P1 v4.1] Single batch processing (batch_size={batch_size}) - formal batch dimension only")
            Pipeline.execute_single_batch(0)
    
    @staticmethod
    def execute_single_batch(batch_idx):
        """Execute single batch of the pipeline (v4.1)
        Note: batch_idx=0 for current single-batch implementation (formal batch dimension)"""
        print(f"[P1 v4.1] Processing batch {batch_idx} with Sionna compatibility")
        ...
        if p1_config.BATCH_PROCESSING:
            metadata["batch_info"] = {
                "batch_index": batch_idx,
                "total_batches": p1_config.BATCH_SIZE,
                "sionna_format": p1_config.USE_SIONNA_FORMAT
            }
        ...
        if p1_config.ENABLE_RAY_GENERATION:
            if p1_config.ENABLE_RAY_SAVING:
    ...
    def execute_integrated_workflow(scene, memory_path_data, run_config):
        ...
        for area_idx, area_index in enumerate(areas):
            ...
            for freq_idx, frequency_ghz in enumerate(frequencies):
                ...
                if p1_config.ENABLE_RAY_GENERATION and path_data_dict:
    ...
    def process_frequency_memory_only(scene, frequency_ghz, area_index):
        ...
        a_, theta_r, phi_r, theta_t, phi_t = PathRT.compute_angles(a_, tau, paths)
        ...
        sorted_data = PathRT.sort_by_power(a_, tau, theta_r, phi_r, theta_t, phi_t, 
                                         path_types_reshaped, vertices_reshaped)
        selected_data = PathRT.select_top(sorted_data, p1_config.NUM_INTERESTING_PATHS)
    ...
    @staticmethod
    def execute_part2():
        """Execute complete path to ray conversion workflow with multi-area and multi-frequency support (Part 2 Cell 2 + v4 enhancement)"""
        
        if p1_config.ENABLE_RAY_GENERATION:
            print("\n--- PART 2: Path to Ray Conversion (Multi-area & Multi-frequency) ---")
            
            # Get processing parameters from v4 configurations
            areas = p1_config.AREA_INDICES
            frequencies = p1_config.FREQUENCY_CONFIGS
            ...
            for area_index in areas:
                ...
                for freq_ghz in frequencies:
                    for rx_index in range(1, total_rx + 1):
                        ...
                        # Load path data using v4 format
                        path_data = DataIO.load_paths_v4(area_index, freq_ghz, rx_index)
                        ...
                        # Save ray data if enabled (v4: unified folder)
                        DataIO.save_rays_v4(ray_arrays, area_index, freq_ghz, rx_index)
    ...
    def process_frequency(scene, frequency_ghz, area_index):
        ...
        # Save results if enabled (v4: Unified folder structure for all/selected RX)
