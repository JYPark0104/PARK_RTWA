...
class P1D_Config:
    ...
    def __init__(self):
        ...
        # 채널 생성 파라미터  
        self.static_ch_realizations = 512 # 독립 정적 채널 개수 (최대값)
        ...
        # 수렴성 체크포인트 (log2 기반, eps_d/eps_U 성공 시 조기 종료)
        self.early_stop_checkpoints = [64, 128, 256]        # 조기 종료 시도 지점
...
class SeparabilityAnalyzer:
    ...
    def _eigendecomposition_jit(self, R_AE_tf, R_BS_tf, R_UE_tf):
        """
        고유값 분해 및 내림차순 정렬 (Direct Error Handling)
        
        반환: 정렬된 고유값/고유벡터 또는 None (실패 시)
        
        Note: Eager mode (try-except 사용)
        - 체크포인트마다만 호출 (~8회/4096 Static)
        - EVD 자체가 병목이므로 Python overhead 무시 가능
        - Diagonal loading (λ=1e-6·tr(R)/d, total trace +0.0001%) for numerical stability
        """
        try:
            # Regularization (Tikhonov diagonal loading): trace의 상대적 비율
            reg_strength = 1e-6  # trace 대비 0.0001% (보수적, 왜곡 최소화)
            
            dim_AE = tf.shape(R_AE_tf)[-1]
            dim_BS = tf.shape(R_BS_tf)[-1]
            dim_UE = tf.shape(R_UE_tf)[-1]
            
            reg_AE = reg_strength * tf.math.real(tf.linalg.trace(R_AE_tf)) / tf.cast(dim_AE, tf.float32)
            reg_BS = reg_strength * tf.math.real(tf.linalg.trace(R_BS_tf)) / tf.cast(dim_BS, tf.float32)
            reg_UE = reg_strength * tf.math.real(tf.linalg.trace(R_UE_tf)) / tf.cast(dim_UE, tf.float32)
            
            R_AE_reg = R_AE_tf + tf.cast(reg_AE, R_AE_tf.dtype) * tf.eye(dim_AE, dtype=R_AE_tf.dtype)
            R_BS_reg = R_BS_tf + tf.cast(reg_BS, R_BS_tf.dtype) * tf.eye(dim_BS, dtype=R_BS_tf.dtype)
            R_UE_reg = R_UE_tf + tf.cast(reg_UE, R_UE_tf.dtype) * tf.eye(dim_UE, dtype=R_UE_tf.dtype)
            
            # 고유값 분해
            lambda_AE_tf, U_AE_tf = tf.linalg.eigh(R_AE_reg)
            lambda_BS_tf, U_BS_tf = tf.linalg.eigh(R_BS_reg)
            lambda_UE_tf, U_UE_tf = tf.linalg.eigh(R_UE_reg)
            ...
            # NaN/Inf 체크
            if (tf.reduce_any(tf.math.is_nan(lambda_AE_tf)) or 
                tf.reduce_any(tf.math.is_inf(lambda_AE_tf)) or
                tf.reduce_any(tf.math.is_nan(lambda_BS_tf)) or 
                tf.reduce_any(tf.math.is_inf(lambda_BS_tf)) or
                tf.reduce_any(tf.math.is_nan(lambda_UE_tf)) or 
                tf.reduce_any(tf.math.is_inf(lambda_UE_tf))):
                return None
            ...
            # EVD 실패 (LAPACK 에러, 샘플 부족, 수치 불안정 등)
            error_msg = str(e)
            if "INVALID_ARGUMENT" in error_msg or "info =" in error_msg:
                # LAPACK heevd 실패 (주로 샘플 부족으로 인한 ill-conditioned matrix)
                pass  # 상위에서 None 체크로 처리
            else:
                # 예상치 못한 에러는 로깅
                print(f"    [EVD 예외] {error_msg[:80]}")
            return None
    ...
    def warmup_jit_functions(self):
        ...
        try:
            ...
            # === 2. EVD 워밍업 (스킵: eager mode로 전환됨) ===
            # Note: _eigendecomposition_jit은 @tf.function 없이 eager mode로 실행
            # Direct error handling을 위해 JIT 컴파일 비활성화
    ...
    def compute_eigen_data(self, covariance_data):
        ...
        # 고유값 분해 실행 (Direct Error Handling)
        evd_result = self._eigendecomposition_jit(R_AE_tf, R_BS_tf, R_UE_tf)
        
        if evd_result is None:
            return {'status': 'failed', 'reason': "실패(EVD)"}
        
        lambda_AE_desc_tf, lambda_BS_desc_tf, lambda_UE_desc_tf, \
        U_AE_desc_tf, U_BS_desc_tf, U_UE_desc_tf = evd_result
    ...
    def compute_separability_metrics(self, eigen_data, kron_data):
        ...
        if valid_rx_mask[0]:
            ...
            # === eps_d 계산 (고유값 오차, underflow 방지) ===
            norm_diff = tf.linalg.norm(lambda_AE_desc_tf - lambda_KM_desc_tf)
            norm_AE = tf.linalg.norm(lambda_AE_desc_tf)
            
            # Safe division (underflow 방지: norm이 매우 작을 때 대응)
            eps_d_desc_tf = norm_diff / (norm_AE + 1e-30)
            ...
            # Column-Major vec 기준: R_AE = R_BS^T ⊗ R_UE (P1E와 동일)
            R_KM_tf = kron_mat_py_tf(tf.transpose(R_b_tf), R_u_tf)
            ...
            # === 랭크 계산 (평균 고유값 기준, path loss 무관) ===
            tol_rel = 1e-3  # 평균 고유값 대비 상대적 threshold (0.1%)
            
            # 평균 고유값 = trace / dim
            mean_lambda_AE = tf.reduce_sum(lambda_AE_desc_tf) / tf.cast(tf.shape(lambda_AE_desc_tf)[0], tf.float32)
            mean_lambda_KM = tf.reduce_sum(lambda_KM_desc_tf) / tf.cast(tf.shape(lambda_KM_desc_tf)[0], tf.float32)
            mean_lambda_BS = tf.reduce_sum(eigen_data['lambda_BS_desc_tf'][0]) / tf.cast(tf.shape(eigen_data['lambda_BS_desc_tf'][0])[0], tf.float32)
            mean_lambda_UE = tf.reduce_sum(eigen_data['lambda_UE_desc_tf'][0]) / tf.cast(tf.shape(eigen_data['lambda_UE_desc_tf'][0])[0], tf.float32)
            
            tol_AE = tol_rel * mean_lambda_AE
            tol_KM = tol_rel * mean_lambda_KM
            tol_BS = tol_rel * mean_lambda_BS
            tol_UE = tol_rel * mean_lambda_UE
            
            AE_rank_np[0] = int(tf.reduce_sum(tf.cast(tf.abs(lambda_AE_desc_tf) > tol_AE, tf.int32)).numpy())
            KM_rank_np[0] = int(tf.reduce_sum(tf.cast(tf.abs(lambda_KM_desc_tf) > tol_KM, tf.int32)).numpy())
            BS_rank_np[0] = int(tf.reduce_sum(tf.cast(tf.abs(eigen_data['lambda_BS_desc_tf'][0]) > tol_BS, tf.int32)).numpy())
            UE_rank_np[0] = int(tf.reduce_sum(tf.cast(tf.abs(eigen_data['lambda_UE_desc_tf'][0]) > tol_UE, tf.int32)).numpy())
...
class ChannelAnalyzer:
    ...
    def compute_checkpoint_metrics(self, accumulated_sample_count):
        """체크포인트 중간 분리성 메트릭 계산 (P1E 검증 완료 패턴)
        
        Parameters:
            accumulated_sample_count: 현재까지 누적된 총 OFDM 샘플 수
            
        Returns:
            dict: 체크포인트 메트릭 (eps_d, eps_U, R_sep_err 등)
        """
        # 1. 현재 누적 공분산 계산
        ...
        R_AE_sam = temp_covs['R_AE_sam']
        R_BS_sam = temp_covs['R_BS_sam']
        R_UE_sam = temp_covs['R_UE_sam']
        ...
        # 3. R_AE - R_KM 크로네커 오차 계산 (P1E 검증 완료 패턴)
        R_AE_sam_2d = R_AE_sam[0]
        R_BS_sam_2d = R_BS_sam[0]
        R_UE_sam_2d = R_UE_sam[0]

        # Trace 계산 (complex64로 변환)
        R_AE_trace = tf.cast(tf.math.real(tf.linalg.trace(R_AE_sam_2d)), tf.complex64)
        R_BS_trace = tf.cast(tf.math.real(tf.linalg.trace(R_BS_sam_2d)), tf.complex64)
        R_UE_trace = tf.cast(tf.math.real(tf.linalg.trace(R_UE_sam_2d)), tf.complex64)
        
        # Column-Major vec 기준: R_AE = R_BS^T ⊗ R_UE (P1E와 동일)
        R_KM_sam = kron_mat_py_tf(tf.transpose(R_BS_sam_2d), R_UE_sam_2d)
        
        # Trace 정규화된 오차 계산
        diff_matrix = R_AE_sam_2d - R_KM_sam*R_AE_trace/(R_BS_trace*R_UE_trace)
        fro_diff = tf.sqrt(tf.reduce_sum(tf.square(tf.abs(diff_matrix))))
        fro_AE = tf.sqrt(tf.reduce_sum(tf.square(tf.abs(R_AE_sam_2d))))
        R_sep_err_fro_rel = float((fro_diff / (fro_AE + 1e-12)).numpy())
        
        # 4. 결과 반환 (rank 포함)
        return {
            'sample_count': accumulated_sample_count,
            'eps_d': metrics_data_np['epsilon_d_desc'],
            'eps_U': metrics_data_np['epsilon_U_desc'],
            'Inner_MaxAll': metrics_data_np['Inner_MaxAll'],
            'Inner_MinRowMax': metrics_data_np['Inner_MinRowMax'],
            'Inner_MinSumSq': metrics_data_np['Inner_MinSumSq'],
            'R_sep_err_fro_rel': R_sep_err_fro_rel,
...
class P1D_ResultManager:
    ...
    def __init__(self, config):
        ...
        # 체크포인트 CSV 경로: 최신 파일 복사 후 새 타임스탬프로 생성
        import glob
        import shutil
        ...
        self.checkpoint_csv_path = os.path.join(config.P1D_ANALYSIS_OUTPUT_DIR, f"P1D_checkpoint_prog_{timetag}.csv")
        
        # 기존 체크포인트 파일이 있으면 복사 (백업 + 이력 관리)
        existing_checkpoints = glob.glob(os.path.join(config.P1D_ANALYSIS_OUTPUT_DIR, "P1D_checkpoint_prog_*.csv"))
        if existing_checkpoints:
            # 가장 최신 파일 찾기
            latest_checkpoint = sorted(existing_checkpoints)[-1]
            # 새 파일명으로 복사
            shutil.copy2(latest_checkpoint, self.checkpoint_csv_path)
            print(f"이전 체크포인트 복사: {os.path.basename(latest_checkpoint)} → {os.path.basename(self.checkpoint_csv_path)}")
    ...
    def save_checkpoint_progress(self, checkpoint_metrics, area_idx, freq_ghz, rx_idx, realization_idx):
        """체크포인트 메트릭을 실시간 CSV에 append (P1E 검증 완료 패턴)
        
        Parameters:
            checkpoint_metrics: compute_checkpoint_metrics() 결과
            area_idx: Area 인덱스
            freq_ghz: 주파수 (GHz)
            rx_idx: RX 인덱스
            realization_idx: Static 실현 인덱스
        """
        ...
        # P1D checkpoint CSV 형식 (P1E 기반 간소화)
        row_data = {
            'n_t': self.config.n_t,
            'n_r': self.config.n_r,
            'area_idx': area_idx,
            'freq_ghz': freq_ghz,
            'rx_idx': rx_idx,
            'eps_d': format_value(checkpoint_metrics.get('eps_d', np.nan)),
            'eps_U': format_value(checkpoint_metrics.get('eps_U', np.nan)),
            'Inner_MaxAll': format_value(checkpoint_metrics.get('Inner_MaxAll', np.nan)),
            'Inner_MinRowMax': format_value(checkpoint_metrics.get('Inner_MinRowMax', np.nan)),
            'Inner_MinSumSq': format_value(checkpoint_metrics.get('Inner_MinSumSq', np.nan)),
            'R_sep_err_fro_rel': format_value(checkpoint_metrics.get('R_sep_err_fro_rel', np.nan)),
            'R_AE_trace': format_value(checkpoint_metrics.get('R_AE_trace', np.nan), use_scientific=True),
            'AE_rank': checkpoint_metrics.get('AE_rank', np.nan),
            'BS_rank': checkpoint_metrics.get('BS_rank', np.nan),
            'UE_rank': checkpoint_metrics.get('UE_rank', np.nan),
            'KM_rank': checkpoint_metrics.get('KM_rank', np.nan),
            'realization_idx': realization_idx,
            'sample_count': checkpoint_metrics.get('sample_count', 0)
        }
        ...
        column_order = [
            ...
            'R_sep_err_fro_rel', 'R_AE_trace',
            'AE_rank', 'BS_rank', 'UE_rank', 'KM_rank',
            'realization_idx', 'sample_count'
        ]
        df_row = pd.DataFrame([row_data], columns=column_order)
        
        # 기존 파일이 있으면 추가, 없으면 새로 생성
        if os.path.exists(self.checkpoint_csv_path):
            df_row.to_csv(self.checkpoint_csv_path, mode='a', header=False, index=False)
        else:
            df_row.to_csv(self.checkpoint_csv_path, index=False)
    ...
    def save_rx_progress(self, analysis_result):
        ...
        row_data = {
            ...
            'epsilon_d_desc': analysis_result['epsilon_d_desc'],
            'epsilon_d_kron': analysis_result['epsilon_d_desc'],
            'epsilon_U_desc': analysis_result['epsilon_U_desc'],
    ...
    def _save_csv_summary(self, csv_file, valid_results, summary_stats):
        ...
        progress_cols = ['area_idx', 'freq_ghz', 'rx_idx', 'R_AE_trace',
                        'epsilon_d_desc', 'epsilon_d_kron', 'epsilon_U_desc', 'eps_R_fro',
...
def main():
    ...
    print(f"\n=== 주요 결과 파일 ===")
    print(f"체크포인트 CSV (주 결과): {os.path.basename(result_manager.checkpoint_csv_path)}")
    print(f"Progress CSV (요약): {os.path.basename(result_manager.progress_csv_path)}")
    print()
    all_results = []  # 모든 RX 결과를 수집
    
    # 이미 완료된 RX 목록 읽기 (재시작 지원)
    completed_rxs = set()
    if os.path.exists(result_manager.checkpoint_csv_path):
        try:
            import pandas as pd
            existing_df = pd.read_csv(result_manager.checkpoint_csv_path)
            if not existing_df.empty:
                # (area_idx, freq_ghz, rx_idx) 튜플로 완료된 RX 식별
                for _, row in existing_df.iterrows():
                    completed_rxs.add((int(row['area_idx']), float(row['freq_ghz']), int(row['rx_idx'])))
                print(f"=== 재시작 모드: {len(completed_rxs)}개 RX 이미 완료, 이어서 진행 ===\n")
        except Exception as e:
            print(f"경고: 기존 체크포인트 읽기 실패 ({e}), 처음부터 시작합니다.\n")
    ...
    for area_index, fc, RX_index in config.p1b_data_combinations:
        # 이미 완료된 RX는 건너뛰기
        if (area_index, fc, RX_index) in completed_rxs:
            continue
        rx_file_count += 1
        ...
        # Static 루프 - 온라인 누적 (P1E 검증 완료 패턴)
        num_static_realizations = config.static_ch_realizations
        samples_per_realization = config.OFDM_FFT
        
        # 가장 최근 성공한 체크포인트 결과 캐싱 (중복 계산 방지)
        last_success_checkpoint_result = None
        last_success_checkpoint_static_idx = None
        early_stop_min = min(config.early_stop_checkpoints)  # 조기 종료 최소 지점
        has_early_success = False  # 조기 종료 최소값 이전 성공 여부
        
        for static_idx in range(1, num_static_realizations + 1):
            # 채널 생성 및 누적 (0-based 인덱스로 전달)
            channel_analyzer.process_static_realization(static_idx - 1)
            
            # 누적 샘플 수 계산
            accumulated_sample_count = static_idx * samples_per_realization
            
            # 체크포인트 확인 (P1E 검증 완료 패턴)
            if static_idx in config.convergence_check_points:
                # 진행률 및 시간 정보
                elapsed_time = time.time() - start_accumulation
                ...
                progress_info += f" | 경과: {format_time_local(elapsed_time)}, 예상잔여: {format_time_local(estimated_remaining)}"
                
                print(f"\n{progress_info}")
                
                # 체크포인트 메트릭 계산 및 CSV 저장 (P1E 검증 완료 패턴)
                checkpoint_metrics = channel_analyzer.compute_checkpoint_metrics(accumulated_sample_count)
                
                if checkpoint_metrics:
                    # 성공한 체크포인트 결과 캐싱 (최종 분석 시 재사용)
                    last_success_checkpoint_result = checkpoint_metrics
                    last_success_checkpoint_static_idx = static_idx
                    
                    # 실시간 CSV에 저장
                    result_manager.save_checkpoint_progress(
                        checkpoint_metrics, area_index, fc, RX_index, static_idx
                    )
                    
                    # 수렴 상황 출력
                    convergence_info = f"eps_d={checkpoint_metrics['eps_d']:.4f}, eps_U={checkpoint_metrics['eps_U']:.4f}"
                    convergence_info += f", R_sep_err={checkpoint_metrics['R_sep_err_fro_rel']:.2e}"
                    print(f"    수렴상태: {convergence_info}")
                    
                    # 조기 종료 체크 (early_stop_min까지는 무조건 실행, 이후 판단)
                    is_success = not np.isnan(checkpoint_metrics['eps_d']) and not np.isnan(checkpoint_metrics['eps_U'])
                    
                    if is_success and static_idx < early_stop_min:
                        has_early_success = True  # 조기 종료 최소값 이전 성공 기록
                    
                    if static_idx >= early_stop_min and static_idx in config.early_stop_checkpoints:
                        if has_early_success or is_success:  # OR: 이전 성공했거나 현재 성공하면 충분
                            print(f"    ✓ 조기 종료: Static {static_idx}에서 eps_d, eps_U 확인 → 다음 RX로")
                            break  # for loop 종료
                else:
                    # EVD 실패 (샘플 부족 또는 수치 불안정) - 캐시는 업데이트하지 않음
                    print(f"    [스킵] EVD 실패 (샘플={accumulated_sample_count}, 행렬차원={config.n_r*config.n_t})")
            else:
                # 체크포인트 외 간단한 진행 표시 (P1E 방식)
                if static_idx % 100 == 0:
        ...
        # 최종 분석: 체크포인트 CSV가 주 결과물이므로 간소화
        if last_success_checkpoint_result:
            # 성공한 체크포인트 결과를 Progress CSV용으로 변환
            print(f"✓ 최종 결과: 체크포인트 Static {last_success_checkpoint_static_idx} (샘플 {last_success_checkpoint_result['sample_count']:,}개)")
            
            analysis_result = {
                'area_idx': area_index,
                'freq_ghz': fc,
                'rx_idx': RX_index,
                'data_source': f'Checkpoint_S{last_success_checkpoint_static_idx}',
                'R_AE_trace': last_success_checkpoint_result['R_AE_trace'],
                'epsilon_d_desc': last_success_checkpoint_result['eps_d'],
                'epsilon_d_kron': last_success_checkpoint_result['eps_d'],
                'epsilon_U_desc': last_success_checkpoint_result['eps_U'],
                'eps_R_fro': last_success_checkpoint_result['R_sep_err_fro_rel'],
                'AE_rank': last_success_checkpoint_result['AE_rank'],
                'BS_rank': last_success_checkpoint_result['BS_rank'],
                'UE_rank': last_success_checkpoint_result['UE_rank'],
                'KM_rank': last_success_checkpoint_result['KM_rank'],
                'n_r': config.n_r,
                'n_t': config.n_t,
                'n_ofdm_sam_final': last_success_checkpoint_result['sample_count'],
                'static_realizations': config.static_ch_realizations,
                **channel_analyzer.analyzer.determine_channel_model(
                    last_success_checkpoint_result['eps_d'], 
                    last_success_checkpoint_result['eps_U'], 
                    config.n_r, config.n_t)
            }
        else:
            # 성공한 체크포인트가 없는 경우
            print("✗ 모든 체크포인트 실패 - 결과 없음")
        ...
        if analysis_result:
            ...
            print(f"  - 모델: {analysis_result['recommended_model']}, " +
                  f"eps_d={analysis_result['epsilon_d_desc']:.4f}, " +
                  f"eps_U={analysis_result['epsilon_U_desc']:.4f}")
            print(f"  - Progress CSV 업데이트: {os.path.basename(result_manager.progress_csv_path)}")
        else:
            # 분석 실패: 성공한 체크포인트 없음
    ...
    print(f"최대 Static 실현: {config.static_ch_realizations}개 (체크포인트: {len(config.convergence_check_points)}개)")
    ...
    if all_results:
        ...
        # 체크포인트 CSV (주 결과)
        if os.path.exists(result_manager.checkpoint_csv_path):
            checkpoint_count = sum(1 for _ in open(result_manager.checkpoint_csv_path)) - 1
            filesize_kb = os.path.getsize(result_manager.checkpoint_csv_path) / 1024
            print(f"✓ 체크포인트 CSV: {os.path.basename(result_manager.checkpoint_csv_path)}")
            print(f"  - {checkpoint_count}개 체크포인트, {filesize_kb:.1f} KB")
        
        # Progress CSV (요약)
        if os.path.exists(result_manager.progress_csv_path):
            print(f"✓ Progress CSV: {os.path.basename(result_manager.progress_csv_path)}")
            print(f"  - {valid_count}/{len(all_results)}개 RX 요약")
        
        print(f"\n저장 위치: {config.P1D_ANALYSIS_OUTPUT_DIR}")
