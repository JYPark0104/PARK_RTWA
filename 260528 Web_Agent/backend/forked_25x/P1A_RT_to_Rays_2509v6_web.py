...
# Web Agent가 runtime_env로 관리 중이면 덮어쓰지 않음 (5090 cuda_malloc_async 이슈 회피)
if not os.environ.get("WEBAGENT_MANAGED_RUNTIME"):
...
# Web Agent: 이미 backend/__init__.py에서 variant가 설정되어 있으면 그것을 유지.
# 환경변수 WEBAGENT_USE_GPU=1이면 cuda variant가 이미 잡혀 있음. 안 잡혔으면 LLVM으로.
import mitsuba as mi
try:
    _current_variant = mi.variant()
except Exception:
...
class P1A_Config:
    ...
    #
    # [Web Agent Fork]: 외부에서 scene.xml 경로를 직접 주입하기 위한 추가 필드.
    # Scene.find_file()이 이 값을 우선 사용한다.
    WEB_OVERRIDE_SCENE_PATH: str | None = None

    # [Web Agent Fork]: PathSolver flag 사용자 노출 (원본 hardcoded 부분을 옵션화).
    # 25*의 검증값 그대로 디폴트.
    PATHSOLVER_LOS = True
...
class Utils:
    ...
    def get_rx_count_from_config(area_config):
        ...
        if method == 'grid':
            ...
            # [Web Agent Fork] 백엔드가 미리 계산한 (x,y,z) 쌍 좌표 (예: 지면 격자 RX)
            return len(placement_config.get('points', []))
...
class Scene:
    ...
    def find_file():
        ...
        [Web Agent Fork]: WEB_OVERRIDE_SCENE_PATH가 설정되어 있으면 그것을 최우선 사용.
        """
        ...
        if getattr(p1a_config, "WEB_OVERRIDE_SCENE_PATH", None):
            ...
            raise FileNotFoundError(
                f"[Web Agent] WEB_OVERRIDE_SCENE_PATH not found: {override}"
            )
    ...
    def generate_area_rx_grid(area_index):
        ...
        if not placement_config:
            ...
            if method == 'grid':
                ...
                # [Web Agent Fork] 백엔드가 지면 레이캐스팅으로 미리 계산한 (x,y,z) 쌍 좌표.
                #   grid/explicit 의 데카르트 곱과 달리, 좌표를 그대로 사용한다.
                rx_positions = [[float(p[0]), float(p[1]), float(p[2])]
                                for p in placement_config['points']]
                print(f"Area {area_index}: Generated {len(rx_positions)} RX positions using {method_info}.")
                return rx_positions
...
class PathRT:
    ...
    def trace_rays(scene, carrier_frequency, frequency_ghz):
        ...
        p_solver = PathSolver()
        # [Web Agent Fork]: PathSolver flag 외부 옵션화 (p1a_config로 노출). 기본값은 25* 원본과 동일.
        #   PARK_2 config.yaml 이식: diffraction/edge_diffraction/diffraction_lit_region,
        #   samples_per_src(num_samples), max_num_paths_per_src 도 옵션화 (없으면 Sionna 기본/원본값).
        _solver_kwargs = dict(
            scene=scene,
            max_depth=p1a_config.MAX_DEPTH,
        ...
        _num_samples = getattr(p1a_config, "RT_NUM_SAMPLES", None)
        if _num_samples:
            _solver_kwargs["samples_per_src"] = int(_num_samples)
        _max_paths = getattr(p1a_config, "RT_MAX_NUM_PATHS", None)
        if _max_paths:
            _solver_kwargs["max_num_paths_per_src"] = int(_max_paths)
        # diffraction 계열은 Sionna 버전에 따라 인자 유무가 다를 수 있어 try/except 로 보호
        try:
            paths = p_solver(
                **_solver_kwargs,
                diffraction=getattr(p1a_config, "PATHSOLVER_DIFFRACTION", False),
                edge_diffraction=getattr(p1a_config, "PATHSOLVER_EDGE_DIFFRACTION", False),
                diffraction_lit_region=getattr(p1a_config, "PATHSOLVER_DIFFRACTION_LIT_REGION", True),
            )
        except TypeError:
            # 구버전 PathSolver: diffraction 인자 미지원 → 핵심 인자만으로 호출
            paths = p_solver(**_solver_kwargs)
...
# ========================================================================
# WEB AGENT HELPERS (Fork-only additions)
# ========================================================================
# 원본 25*의 함수/클래스는 위에서 그대로 유지. 아래 helper들은 Web Agent에서
# 호출하기 위한 얇은 래퍼이며 25* 원본에 영향을 주지 않는다.


def _apply_scattering_pattern(rm) -> None:
    """p1a_config.SCATTERING_PATTERN 에 따라 RadioMaterial 의 산란 패턴을 설정.

    PARK_2 config.yaml 의 material.scattering_pattern (lambertian|directive|
    backscattering) + alpha 파라미터를 반영. 미지정/오류 시 조용히 건너뛴다.
    (전기적 특성 permittivity/conductivity 는 P1A 의 ITU 재질 모델이 주파수
     의존적으로 산출하므로 의도적으로 덮어쓰지 않는다.)
    """
    name = str(getattr(p1a_config, "SCATTERING_PATTERN", "") or "").lower()
    if not name:
        return
    try:
        if name == "directive":
            rm.scattering_pattern = DirectivePattern(
                alpha_r=int(getattr(p1a_config, "DIRECTIVE_ALPHA_R", 10))
            )
        elif name == "backscattering":
            rm.scattering_pattern = BackscatteringPattern(
                alpha_r=int(getattr(p1a_config, "BACKSCATTERING_ALPHA_R", 20)),
                alpha_i=int(getattr(p1a_config, "BACKSCATTERING_ALPHA_I", 30)),
                lambda_=float(getattr(p1a_config, "BACKSCATTERING_LAMBDA", 0.7)),
            )
        elif name == "lambertian":
            rm.scattering_pattern = LambertianPattern()
    except Exception as exc:  # pragma: no cover
        print(f"   ⚠️  scattering_pattern 적용 실패(무시): {exc}")


def web_apply_config_override(overrides: dict) -> None:
    """런타임에 p1a_config 속성을 일괄 덮어쓰기.

    Web Agent backend 어댑터에서 사용자 옵션(주파수, MAX_DEPTH, PathSolver flag,
    AREA_CONFIGS, UNIFIED_SAVE_DIR, scene path 등)을 주입할 때 호출.

    Example
    -------
    >>> web_apply_config_override({
    ...     "WEB_OVERRIDE_SCENE_PATH": "/sessions/abc/scene/scene.xml",
    ...     "UNIFIED_SAVE_DIR": "/sessions/abc/P1A_RT_Results",
    ...     "MAX_DEPTH": 8,
    ...     "FREQUENCY_CONFIGS": [3.5],
    ...     "AREA_INDICES": [99],
    ...     "AREA_CONFIGS": {"area_99": {...}},
    ... })
    """

    for k, v in overrides.items():
        setattr(p1a_config, k, v)


def web_run_pipeline(overrides: dict | None = None) -> None:
    """Web Agent backend 어댑터용 단일 진입점.

    1) overrides 적용 → 2) Pipeline.execute_main() 호출.
    호출자는 잡 매니저(job_runner)에서 비동기로 ThreadPoolExecutor에 던지면 된다.
    """

    if overrides:
        web_apply_config_override(overrides)
    Pipeline.execute_main()


# ========================================================================
# EXECUTION PIPELINE (CLI mode — original behavior)
# ========================================================================
