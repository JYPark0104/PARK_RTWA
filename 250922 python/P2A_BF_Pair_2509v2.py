...
# P3: Ray Data → 빔포밍 채널 이득 계산
# 원본: P3_BF_Pair_2509v1.py
# 
# 개선사항:
# - 레이어 기반 빔포밍 채널 이득 계산
# - 2차원 크로네커 DFT 코드북 생성
...
# ===== SECTION 2: P3_Config (레이어 기반 빔포밍 설정 확장) =====
class P3_Config:
    """P2 저장 데이터를 기반으로 실험 설정을 동적으로 구성 (레이어 기반 빔포밍)"""
    ...
    def __init__(self):
        ...
        self.P2_INPUT_DIR = os.path.join(script_dir, "P1A_RT_Results")
        self.P2_FILE_PATTERN = "Area{area}_{freq}GHz_Rays_ALL_RXs.npz"  # Updated to unified npz format
        self.P3_OUTPUT_DIR = os.path.join(script_dir, "P2A_BF_Results")
        self.P3_FILE_PATTERN = "Area{area}_{freq}GHz_RX{rx}_layer_beam_data.npz"
        ...
        # fc 리스트를 self.p2_data_combinations에서 추출하여 설정
        if self.p2_data_combinations:
            self.fcs = sorted(list(set([item[1] for item in self.p2_data_combinations])))
        else:
            self.fcs = []
    ...
    def detect_p2_data(self):
        """P2 저장 데이터를 스캔하여 실험 설정 자동 감지 (unified npz format)"""
        
        # P2 출력 디렉토리에서 통합 npz 파일들 스캔
        scan_pattern = f"{self.P2_INPUT_DIR}/{self.P2_FILE_PATTERN.format(area='*', freq='*')}"
        ...
        # 정렬하여 리스트로 저장
        self.p2_data_combinations = sorted(list(combinations))
        
        print(f"P2 데이터 자동 감지 (unified npz format):")
        print(f"  - 감지된 조합 수: {len(self.p2_data_combinations)}")
        
        # Area별로 그룹핑하여 출력
        area_groups = {}
        for combo in self.p2_data_combinations:
            area, freq, rx = combo
    ...
    def load_p2_ray_data(self, area_index, frequency, rx_index):
        ...
        npz_filename = self.P2_FILE_PATTERN.format(area=area_index, freq=frequency)
        npz_filepath = os.path.join(self.P2_INPUT_DIR, npz_filename)
    ...
    def create_chunk_from_files(self, area_index, frequency, rx_indices):
        ...
        for idx, rx_idx in enumerate(rx_indices, 1):
            individual_file = os.path.join(self.P3_OUTPUT_DIR, 
                                         self.P3_FILE_PATTERN.format(area=area_index, freq=frequency, rx=rx_idx))
        ...
        chunk_filepath = os.path.join(self.P3_OUTPUT_DIR, chunk_filename)
...
class ChCoeGen(Object):
    """P3: Ray → 빔포밍 채널 이득 계산 (독립 정적채널 + 도플러 시변채널)
    ...
    - P3: 빔포밍 채널 이득 계산 → 빔 페어별 이득 + 방향 정보
    """
...
def main():
    """P3: 빔포밍 채널 이득 계산을 통한 TX-RX 빔 페어 성능 분석"""
    ...
    # P2 데이터 기반 동적 루프
    total_rx_files = len(config.p2_data_combinations)
    rx_file_count = 0
    ...
    for area_index, fc, rx_index in config.p2_data_combinations:
    ...
    for area_index, fc in sorted(area_freq_combinations):
        ...
        codebook_filepath = f'{config.P3_OUTPUT_DIR}/{codebook_filename}'
    ...
    for area_index, fc, RX_index in config.p2_data_combinations:
        ...
        # P2 데이터 로딩 (unified npz format)
        ray_data = config.load_p2_ray_data(area_index, fc, RX_index)
        if ray_data is None:
        ...
        os.makedirs(config.P3_OUTPUT_DIR, exist_ok=True)
        output_filename = config.P3_FILE_PATTERN.format(area=area_index, freq=fc, rx=RX_index)
        ...
        # 간소화된 npz 저장
        np.savez(f'{config.P3_OUTPUT_DIR}/{output_filename}',
    ...
    if config.ENABLE_POST_CHUNK:
        print("\n청크 생성 시작...")
        area_freq_groups = {}
        for area_index, frequency, rx_index in config.p2_data_combinations:
